...
function build(previousFileSizes, config, prefix) {
  console.log(`${prefix}Creating an optimized production build...`);
...
measureFileSizesBeforeBuild(paths.appBuild)
  .then((previousFileSizes) => {
    ...
    copyPublicFolder();
    // Start the webpack build and compile the client
    return build(previousFileSizes, configClient, '[Client] ');
  })
  .then(({ stats, previousFileSizes, warnings }) => {
    ...
    // Compile now the server
    return build(previousFileSizes, configServer, '[Server] ');
  })
  .then(({ stats, previousFileSizes, warnings }) => {
    if (warnings.length) {
